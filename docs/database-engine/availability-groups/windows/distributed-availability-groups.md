...
> [!NOTE]
> Distributed availability groups can be configured with only Enterprise Edition.
...
3. As with a normal availability group, fail over the primary availability group to one of its own replicas (not to the primary of the second availability group) and patch it. If there is no replica other than the primary, a manual failover to the second availability group will be necessary.
